...
class DetailTempSeite(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._dialog = None
        self._validierte_felder = {}  # name -> {"input": QLineEdit, "fehler": QLabel}
        self._eingaben_cache = {}  # WJ-Nummer -> Dict mit gespeicherten Eingaben
        self._erstelle_ui()
    ...
    def _form_label(self, text, tooltip=None):
        ...
        container = QWidget()
        container.setObjectName("detailScrollInhalt")  # erbt transparent vom globalen Stylesheet
    ...
    def _finde_existierendes_pdf(self, wj_nummer):
        """Sucht im Protokoll-Ordner nach einem vorhandenen Temp-PDF fuer diese Waage."""
        if not wj_nummer:
            return None
        protokoll_pfad = self._settings.get_protokoll_path()
        if not protokoll_pfad or not os.path.isdir(protokoll_pfad):
            return None
        treffer = glob.glob(os.path.join(protokoll_pfad, f"TK_*_{wj_nummer}.pdf"))
        if treffer:
            # Neuestes nehmen (nach Änderungsdatum)
